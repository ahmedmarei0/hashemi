...
class UploadFilesHelper
{
    ...
    public static function generate_unique_filename($file,$options){
        ...
        $filename = pathinfo($original_file_name,PATHINFO_FILENAME) . '-' . substr(str_shuffle("0123456789abcdefghijklmnopqrstvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, 10) .'.'. $extension ;
        return $filename;
    }
    public static function upload_image($file, $destination){
       try{
            ...
            $fileName = time().rand(1000,100000).'.'.$extension;
            $file->move(public_path(env('STORAGE_URL').$destination),$fileName);
            ...
            return ['success' => false, 'filename' => $ex->getMessage()];
        }
